...
let schemaTypes = require('./schema-types'),
...
Schema.prototype.toMapping = function () {
  ...
  let checkLeaf = function (obj, path) {
    _.forOwn(obj, function (v, k) {
      let currentPath = path ? path + '.' + k : k;
      if (v.type) {
        let field = {type: v.type.esType};
...
Schema.prototype.validate = function (doc, partial) {
  ...
  let compareWithOptions = function (field, fieldType, path) {
    if (field) {
  ...
  let checkType = function (fieldType, path) {
    let docField = dot.pick(path, doc);
...
function buildFieldsFromSchema(fields) {
  ...
  function mapSchemaType(key, path) {
    ...
    let value = dot.pick(currentPath, fields);
    let schemaType;
    let typeOptions = {};

...
module.exports = {
  ...
  create: function (data, options) {
    ...
    return self.db.client.index({
      index: self.db.index+'-'+self.model.type,
      type: '_doc',
      body: data,
  ...
  find: function (match, queryOptions) {
    ...
    let query = Query.parseRequest(self.db.index +'-'+self.model.type, '_doc', match, queryOptions);
      ...
      .then(function (results) {
        ...
        let bulkOps = _.chain(results)
          ...
          .map(function (v) {
            return {
              'delete': {
                '_index': self.db.index+'-'+self.model.type,
                '_type': '_doc',
                '_id': v.id
              }
            };
  ...
  findById: function (id, queryOptions) {
    ...
    let query = Query.parseRequest(self.db.index+'-'+self.model.type, '_doc', null, queryOptions);
  ...
  remove: function (id, options) {
    ...
    return this.db.client.delete({
      index: this.db.index +'-'+this.model.type,
      refresh: options.refresh || false,
      type: '_doc',
      id: id
    });

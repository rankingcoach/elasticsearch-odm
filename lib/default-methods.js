...
module.exports = {
  ...
  /**
   * same as find but replace generated query by a raw query
   * @param query
   * @param queryOptions
   * @returns {QueryPromise}
   */
  raw: function(query, queryOptions) {
    var self = this;
    queryOptions = _.isPlainObject(queryOptions) ? queryOptions : {};
    var parsedQuery = Query.parseRequest(self.db.index, self.model.type, {}, queryOptions);

    return new Query.QueryPromise(parsedQuery, function(q){
      q.body.query = query;
      return self.search(queryOptions, q);
    });
  },
